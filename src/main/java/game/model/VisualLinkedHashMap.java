...
public class VisualLinkedHashMap {
    ...
    public synchronized Object put(Object key, Object value, String keyType, String valueType) {
        ...
        lastOperation = "PUT: " + key + " -> " + value + " (order: " + insertionOrder.size() + ")";
    ...
    public synchronized Object get(Object key) {
        ...
        for (VisualEntry entry : buckets[bucketIndex]) {
            if (!entry.isRemoving() && entry.getKey().equals(key)) {
                lastOperation = "GET: " + key + " -> " + entry.getValue();
    ...
    public synchronized boolean containsKey(Object key) {
        ...
        for (VisualEntry entry : buckets[bucketIndex]) {
            if (!entry.isRemoving() && entry.getKey().equals(key)) {
                lastOperation = "CONTAINS KEY: " + key + " -> true";
                return true;
            }
        }
    ...
    private void rehash() {
        ...
        List<VisualEntry> oldOrder = new ArrayList<>(insertionOrder);
        insertionOrder.clear();
        size = 0;

        for (VisualEntry entry : oldOrder) {
            if (!entry.isRemoving()) {
    ...
    public synchronized void update(double deltaTime) {
        ...
        insertionOrder.removeIf(VisualEntry::isFullyRemoved);
    ...
    public MemoryInfo getMemoryInfo() {
        ...
        long totalUsed = linkedHashMapOverhead + tableOverhead + nodesOverhead + keysMemory + valuesMemory;

        return new MemoryInfo(totalUsed, linkedHashMapOverhead, tableOverhead, nodesOverhead, keysMemory, valuesMemory);
    }
    ...
    public static Object generateRandomKey(String type, Random random) {
        return VisualHashMap.generateRandomKey(type, random);
    }
