...
public class VisualHashSet {
    ...
    public synchronized boolean add(Object value, String type) {
        ...
        size++;
        operationsCount++;
        lastOperation = "ADD: " + value + " -> bucket[" + bucketIndex + "]";
    ...
    public synchronized boolean remove(Object value) {
        ...
        if (buckets[bucketIndex].removeElement(value)) {
            size--;
            operationsCount++;
            lastBucketAccessed = bucketIndex;
            buckets[bucketIndex].highlight();
    ...
    private void rehash() {
        ...
        for (VisualBucket oldBucket : oldBuckets) {
            for (VisualElement elem : oldBucket.getChain()) {
                if (!elem.isRemoving()) {
    ...
    public MemoryInfo getMemoryInfo() {
        ...
        long wastedMemory = (long) emptyBuckets * 4;

        long totalUsed = hashSetOverhead + tableOverhead + nodesOverhead + elementsMemory;

        return new MemoryInfo(totalUsed, hashSetOverhead, tableOverhead, nodesOverhead, elementsMemory, wastedMemory, emptyBuckets);
    }
    ...
    public record MemoryInfo(
        ...
        public String formatElements() { return formatBytes(elementsMemory); }
