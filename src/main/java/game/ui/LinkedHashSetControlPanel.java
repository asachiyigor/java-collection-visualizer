...
public class LinkedHashSetControlPanel extends JPanel {
    ...
    public LinkedHashSetControlPanel(VisualLinkedHashSet linkedHashSet) {
    ...
    private void initComponents() {
        ...
        for (int i = 0; i < types.length; i++) {
            ...
            btn.addActionListener(e -> {
                Object value = VisualLinkedHashSet.generateRandomValue(type, random);
                if (linkedHashSet.add(value, type)) {
        ...
        sizeBtn.addActionListener(e -> updateStatus("size() = " + linkedHashSet.getSize(), TEXT_COLOR));
        methodPanel.add(sizeBtn);

        JButton isEmptyBtn = createSmallButton("isEmpty()", new Color(150, 200, 255));
        isEmptyBtn.addActionListener(e -> updateStatus("isEmpty() = " + (linkedHashSet.getSize() == 0), TEXT_COLOR));
        ...
        containsBtn.addActionListener(e -> {
            ...
            if (!val.isEmpty()) {
                ...
                try {
                    ...
                    boolean found = linkedHashSet.contains(value);
        ...
        removeBtn.addActionListener(e -> {
            ...
            if (!val.isEmpty()) {
                ...
                try {
                    Object value = parseValue(val, type);
                    if (linkedHashSet.remove(value)) {
                        updateStatus("Removed: " + val, SUCCESS_COLOR);
                        valueField.setText("");
                    } else {
                        updateStatus("Not found", ERROR_COLOR);
                    }
                } catch (Exception ex) {
                    updateStatus("Invalid value", ERROR_COLOR);
                }
        ...
        JButton fill10Btn = createSmallButton("Fill x10", new Color(200, 120, 120));
        fill10Btn.addActionListener(e -> autoFill(10));
        autoPanel.add(fill10Btn);

        JButton fill20Btn = createSmallButton("Fill x20", new Color(220, 140, 140));
    ...
    private void addElement() {
        ...
        if (valueText.isEmpty()) {
            value = VisualLinkedHashSet.generateRandomValue(type, random);
        } else {
            try {
    ...
    private JPanel createInfoPanel() {
        ...
        JLabel title = new JLabel("LINKEDHASHSET INFO");
        ...
        String[] info = {
                ...
                "",
                "Extra 8 bytes per entry",
                "for prev/next pointers"
        };

        for (String line : info) {
            JLabel label = new JLabel(line.isEmpty() ? " " : line);
            label.setForeground(line.contains("order") ? new Color(255, 180, 100) : new Color(200, 180, 185));
