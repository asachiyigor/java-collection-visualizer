...
public class TreeSetControlPanel extends JPanel {
    ...
    public TreeSetControlPanel(VisualTreeSet treeSet) {
    ...
    private void initComponents() {
        ...
        for (int i = 0; i < types.length; i++) {
            ...
            btn.addActionListener(e -> {
                Object value = VisualTreeSet.generateRandomValue(type, random);
                if (treeSet.add(value, type)) {
        ...
        add(createTitle("TREESET METHODS"));
        add(Box.createVerticalStrut(8));
        ...
        sizeBtn.addActionListener(e -> updateStatus("size() = " + treeSet.getSize(), TEXT_COLOR));
        methodPanel.add(sizeBtn);

        JButton firstBtn = createSmallButton("first()", new Color(150, 255, 200));
        firstBtn.addActionListener(e -> {
            Object first = treeSet.first();
            updateStatus("first() = " + (first != null ? first : "empty"), first != null ? SUCCESS_COLOR : WARN_COLOR);
        });
        methodPanel.add(firstBtn);

        JButton lastBtn = createSmallButton("last()", new Color(150, 255, 200));
        lastBtn.addActionListener(e -> {
            Object last = treeSet.last();
            updateStatus("last() = " + (last != null ? last : "empty"), last != null ? SUCCESS_COLOR : WARN_COLOR);
        });
        ...
        containsBtn.addActionListener(e -> {
            ...
            if (!val.isEmpty()) {
                ...
                try {
                    ...
                    boolean found = treeSet.contains(value);
        ...
        JButton fill10Btn = createSmallButton("Fill x10", new Color(80, 180, 180));
        fill10Btn.addActionListener(e -> autoFill(10));
        autoPanel.add(fill10Btn);

        JButton fill20Btn = createSmallButton("Fill x20", new Color(100, 200, 200));
    ...
    private void autoFill(int count) {
        ...
        timer.addActionListener(evt -> {
            if (added[0] < count) {
                Object value = VisualTreeSet.generateRandomValue("int", random);
                treeSet.add(value, "int");
                added[0]++;
                updateStatus("Adding... " + added[0] + "/" + count, SUCCESS_COLOR);
            } else {
                timer.stop();
                updateStatus("Added " + count + " (h=" + treeSet.getHeight() + ")", SUCCESS_COLOR);
            }
        });
        timer.start();
    }
    ...
    private void addElement() {
        ...
        if (valueText.isEmpty()) {
            value = VisualTreeSet.generateRandomValue(type, random);
        } else {
            try {
        ...
        if (treeSet.add(value, type)) {
            updateStatus("Added: " + value + " (h=" + treeSet.getHeight() + ")", SUCCESS_COLOR);
            valueField.setText("");
        } else {
    ...
    private Object parseValue(String text, String type) {
        // Note: TreeSet does NOT allow null elements - throws NPE
    ...
    private JPanel createInfoPanel() {
        ...
        JLabel title = new JLabel("TREESET INFO");
        ...
        for (String line : info) {
            ...
            label.setForeground(line.contains("sorted") ? SUCCESS_COLOR : new Color(150, 200, 200));
